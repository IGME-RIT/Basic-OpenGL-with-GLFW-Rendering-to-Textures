...
GLuint LoadTexturePNG(const char* file)
{
	...
	FREE_IMAGE_FORMAT fileFormat = FreeImage_GetFileType(file);
	// Load the file.
	FIBITMAP* bitmap = FreeImage_Load(fileFormat, file);
...
int main(int argc, char **argv)
{
	...
	// load texture that we want to draw
	GLuint texture = LoadTexturePNG("../assets/texture.png");

    // Create shader program:
    Shader vertexShader, fragmentShader;
    
	// Initialize the shaders using the shader files.
	vertexShader.InitFromFile("../assets/1stVS.glsl", GL_VERTEX_SHADER);
	fragmentShader.InitFromFile("../assets/1stFS.glsl", GL_FRAGMENT_SHADER);
    ...
    fullscreenVert.InitFromFile("../assets/2ndVS.glsl", GL_VERTEX_SHADER);
    invertFrag.InitFromFile("../assets/2ndFS.glsl", GL_FRAGMENT_SHADER);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// Draw Square in the first render pass
		// ====================================
		...
		// Take a screenshot, and then alter it
		// ====================================
